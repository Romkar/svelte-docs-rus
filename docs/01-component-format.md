...
Svelte использует ключевое слово `export`, чтобы пометить объявление переменной как _свойство_, что означает, что оно становится доступным извне всему, что будет использовать этот компонент (см. раздел [Атрибуты и свойства](/docs#template-syntax-attributes-and-props) для дополнительной информации).
...
Доступ к переменным только для чтения можно получить в качестве свойств элемента, привязанных к компоненту с помощью синтаксиса [`bind:this`](/docs#template-syntax-element-directives-bind-element).
...
[Модуль `svelte/store`](/docs#run-time-svelte-store) содержит минимальную реализацию хранилищ, соответствующих такому контракту.
...
Вы можете создавать собственные варианты хранилищ, без участия [`svelte/store`](/docs#run-time-svelte-store), реализовав _контракт хранилища_:
...
> [хранилища](/docs#run-time-svelte-store).
