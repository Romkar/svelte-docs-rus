...
Все остальные атрибуты будут присоединены к элементу, только если их значение не является [null-образным](https://developer.mozilla.org/en-US/docs/Glossary/Nullish) (`null` или `undefined`).
...
> при [создании компонента](/docs/04-run-time?id=api-компонента-на-клиенте).
...
Если переход возвращает функцию вместо объекта перехода, то она будет вызвана в следующей микрозадаче. Это позволяет координировать несколько переходов, что дает возможность запускать [перекрёстные переходы](https://svelte.dev/tutorial/deferred-transitions).
...
Анимация запускается при изменении порядка содержимого [блока each с ключом](/docs/03-template-syntax?id=each-). Анимация не запускается при добавлении или удалении элемента, только при изменении индекса существующего элемента данных в каждом блоке. Анимационные директивы должны быть на элементе, который является *непосредственным* дочерним элементом каждого блока с ключом.

Анимация может использовать [встроенные функции анимации](/docs/04-run-time?id=svelteanimate) или [пользовательские функции анимации](/docs/03-template-syntax?id=animatefn).
...
Компоненты могут отправлять события, используя диспетчер событий [createEventDispatcher](/docs/04-run-time?id=createeventdispatcher) или пробрасывая события DOM. Обработка событий компонента выглядит так же, как обработка событий DOM.
...
Подобно `<svelte:window>`, этот элемент позволяет добавлять обработчики событий на `document.body`, такие как `mouseenter` и `mouseleave`, которые не запускаются на `window`. Он также позволяет использовать [actions](/docs/03-template-syntax?id=useдействие) для элемента `<body>`.
...
Элемент `<svelte:options>` позволяет установить определенные параметры для компилятора для каждого отдельного компонента. Подробнее о параметрах компилятора можно узнать в разделе про функцию [compile](/docs/05-compile-time?id=sveltecompile). Параметры, которые можно установить:
...
Элемент `<svelte:fragment>` позволяет размещать контент в [именованных слотах](/docs/03-template-syntax?id=ltslotgt) не оборачивая его в дополнительный элемент DOM. Это сохраняет структуру макета вашего документа.
